...
**Goal:** Update `codex-setup` so it can install the current live `~/.codex` setup onto a Windows machine as a one-to-one functional mirror, excluding native notification and permission-request notification hooks.

**Architecture:** Treat `/Users/gabrielmagno/.codex` as the source of truth and `codex-setup/home` as a sanitized, portable snapshot. A snapshot script copies durable setup artifacts, strips native notification behavior and machine runtime state, keeps the Stop ding hook, replaces Mac-only absolute paths with `__CODEX_HOME__`, and the Windows installer materializes that placeholder as `$env:USERPROFILE\.codex`.
...
- Current repo snapshot has `48` skill directories under `codex-setup/home/skills`.
- Live setup has `94` files under `/Users/gabrielmagno/.codex/agents`.
- Current repo snapshot has `46` files under `codex-setup/home/agents`.
...
- Create `codex-setup/scripts/snapshot-codex.sh`
  - Refreshes `codex-setup/home` from live `/Users/gabrielmagno/.codex`.
  - Copies only durable setup artifacts.
  - Calls the sanitizer and verifier.

- Create `codex-setup/scripts/sanitize-codex-home.mjs`
...
- Create `codex-setup/scripts/verify-codex-home.mjs`
...
- Modify `codex-setup/scripts/setup-codex.ps1`
...
- Modify `codex-setup/scripts/setup-codex.sh`
  - Materializes `__CODEX_HOME__` after copying.
...
**Files:**
- Create: `codex-setup/scripts/sanitize-codex-home.mjs`
- Test: `codex-setup/scripts/verify-codex-home.mjs` is added in Task 2

- [ ] **Step 1: Create the sanitizer**

Create `codex-setup/scripts/sanitize-codex-home.mjs` with this content:
...
```bash
chmod +x codex-setup/scripts/sanitize-codex-home.mjs
```
...
```bash
node --check codex-setup/scripts/sanitize-codex-home.mjs
```
...
```bash
git add codex-setup/scripts/sanitize-codex-home.mjs
...
Create `codex-setup/scripts/verify-codex-home.mjs` with this content:
...
```bash
chmod +x codex-setup/scripts/verify-codex-home.mjs
```
...
```bash
node --check codex-setup/scripts/verify-codex-home.mjs
```
...
```bash
git add codex-setup/scripts/verify-codex-home.mjs
...
**Files:**
- Create: `codex-setup/scripts/snapshot-codex.sh`
- Modify: `codex-setup/home/`
- Test: `codex-setup/scripts/verify-codex-home.mjs`

- [ ] **Step 1: Create the snapshot script**

Create `codex-setup/scripts/snapshot-codex.sh` with this content:
...
```bash
chmod +x codex-setup/scripts/snapshot-codex.sh
```
...
```bash
bash -n codex-setup/scripts/snapshot-codex.sh
```
...
```bash
codex-setup/scripts/snapshot-codex.sh
```
...
```bash
find codex-setup/home/skills -maxdepth 1 -mindepth 1 -type d | wc -l
find codex-setup/home/agents -maxdepth 1 -type f | wc -l
test -d codex-setup/home/plugins/cache/openai-curated/superpowers
test -d codex-setup/home/get-shit-done
```
...
```bash
git add codex-setup/scripts/snapshot-codex.sh codex-setup/home
...
In `codex-setup/scripts/setup-codex.ps1`, after this existing line:
...
```bash
pwsh -NoProfile -Command '$null = [System.Management.Automation.PSParser]::Tokenize((Get-Content -Raw "codex-setup/scripts/setup-codex.ps1"), [ref]$null); "OK"'
```
...
```bash
git add codex-setup/scripts/setup-codex.ps1
...
In `codex-setup/scripts/setup-codex.sh`, after:
...
```bash
bash -n codex-setup/scripts/setup-codex.sh
```
...
```bash
git add codex-setup/scripts/setup-codex.sh
...
codex-setup/private-home/
codex-setup/**/*.private.*
codex-setup/**/auth.json
codex-setup/**/.env
```
...
```bash
mkdir -p codex-setup/private-home
touch codex-setup/private-home/auth.json
git check-ignore codex-setup/private-home/auth.json
rm codex-setup/private-home/auth.json
rmdir codex-setup/private-home
```

Expected:

```text
codex-setup/private-home/auth.json
```
...
In `codex-setup/README.md`, replace the `What is in home` table with:
...
In `codex-setup/README.md`, ensure the Windows install section says:
...
Add this section near the top of `codex-setup/BOOTSTRAP.md`:
...
Add this section to `codex-setup/MIGRATION_REPORT.md`:
...
```bash
git add codex-setup/README.md codex-setup/BOOTSTRAP.md codex-setup/MIGRATION_REPORT.md
...
**Files:**
- Verify: `codex-setup/home`
- Verify: `codex-setup/scripts/setup-codex.ps1`
- Verify: `codex-setup/scripts/setup-codex.sh`
...
```bash
rg "notify\\.sh|ding\\.sh|PermissionRequest|notification_method|notification_condition" codex-setup/home
```
...
```bash
rg "/Users/gabrielmagno/\\.codex" codex-setup/home
```
...
```bash
rg "__CODEX_HOME__" codex-setup/home/config.toml
```
...
```bash
pwsh -NoProfile -File codex-setup/scripts/setup-codex.ps1 -DryRun
```
...
```bash
codex-setup/scripts/setup-codex.sh --dry-run
```
...
Expected: only intended files changed. Existing pre-plan user changes under `codex-setup/home` are expected to be replaced by the refreshed live snapshot because the live setup is the source of truth.
